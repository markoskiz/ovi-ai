...
def Heuristika(tabla,z):
    ...
    #vertikalna ocena
    tabla_temp=np.array(tabla)
    for k in range(7):
        kolona = tabla_temp[:, k]
        for i in range(len(kolona)-3):
            temp_ocena = 0
            for j in range(i, i + 4):
                if kolona[j] == '_':
                    continue
                if kolona[j] != '_' and kolona[j] != z:
                    temp_ocena = 0
                    break
                if kolona[j] == z:
                    temp_ocena += 1
    ...
    #rastecka dijagonala ocena
    rastecka = []
    a=np.array(tabla)
    diags = [a[::-1, :].diagonal(i) for i in range(-a.shape[0] + 1, a.shape[1])]
    for n in diags:
        rastecka_temp = n.tolist()
        if len(rastecka_temp) >= 4:
            rastecka.append(rastecka_temp)
    for red in rastecka:
        for i in range(len(red)-3):
            temp_ocena = 0
            for j in range(i, i+4):
    ...
    #opagacka dijagonala ocena
    opagacka = []
    a = np.array(tabla)
    diags.extend(a.diagonal(i) for i in range(a.shape[1] - 1, -a.shape[0], -1))
    for n in diags:
        opagacka_temp = n.tolist()
        if len(opagacka_temp) >= 4:
            opagacka.append(opagacka_temp)
    for red in opagacka:
        for i in range(len(red) - 3):
            temp_ocena = 0
            for j in range(i, i + 4):
